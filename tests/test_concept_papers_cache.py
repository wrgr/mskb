...
def _load_valid_paper_ids() -> set[str]:
    # concept_papers.json is generated by link_concepts_to_papers.py from scored_papers.csv
    # filtered to in_final_corpus=True rows. Use that same filter as the authoritative set,
    # supplemented by any IDs visible in the explorer graph (which is a subset of the corpus).
    assert SCORED_PAPERS_PATH.exists(), f"missing {SCORED_PAPERS_PATH}"
    frame = pd.read_csv(SCORED_PAPERS_PATH, usecols=["canonical_paper_id", "in_final_corpus"], low_memory=False)
    valid: set[str] = {
        str(v).strip()
        for v, flag in zip(frame["canonical_paper_id"], frame["in_final_corpus"])
        if str(v).strip() and str(flag).strip().lower() not in {"false", "0", "nan", ""}
    }
    # Also accept any paper that made it into the explorer graph.
    if EXPLORER_GRAPH_PATH.exists():
        ...
        valid.update(str(n[id_idx]).strip() for n in payload.get("nodes", []) if str(n[id_idx]).strip())
    return valid
