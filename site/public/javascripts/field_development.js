...
// Renders a stacked-bar timeline (annual paper output by domain) and a
// scatter/bubble chart (year vs importance, sized by citations) from
// field_development.json produced by the pipeline.
(() => {
  "use strict";
  ...
  async function init() {
    ...
    try {
      ...
      document.querySelectorAll(".fd-cat-filter").forEach(cb => {
        cb.addEventListener("change", () => renderAll(rawData));
      });
      ...
      window.addEventListener("resize", () => {
        ...
        resizeTimer = setTimeout(() => renderAll(rawData), 120);
      });
